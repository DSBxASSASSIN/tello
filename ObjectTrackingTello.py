        ...
        for x in range (0, rows):
            for y in range (0, cols):
                if imgArray[x][y].shape[:2] == imgArray[0][0].shape[:2]:
                    ...
                    imgArray[x][y] = cv2.resize(imgArray[x][y], (imgArray[0][0].shape[0], imgArray[0][0].shape[0]), None, scale, scale)
                if len(imgArray[x][y].shape) == 2: imgArray[x][y] = cv2.cvtColor(imgArray[x][y], cv2.COLOR_BGR2RGB)
    ...
    for cnt in contours:
        ...
        if area > areaMin:
            ...
            if (cx < int(frameWidth / 2) - deadZone):
                ...
                cv2.rectangle(imgContour, (int(frameWidth / 2 - deadZone), int(frameHeight / 2 + deadZone), (int(frameHeight / 2) + deadZone), frameHeight), (0, 0, 255), cv2.FILLED)
                dir = 4
            else: dir = 0
            ...
            cv2.putText(imgContour, "Area: " + str(int(area)), (x + w + 20, y + 20), cv2.FONT_HERSHEY_COMPLEX, .7, (0, 255, 0), 2)
